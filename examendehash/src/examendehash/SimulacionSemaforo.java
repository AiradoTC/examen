               ...
               while (true) {
                   synchronized (lock) {
                       if ("ROJO".equals(estadoActual)) {
                           System.out.println("Rojo:stop por 5 segundos");
                           try {
                               Thread.sleep(5000);
           ...
           Thread hiloVerde = new Thread(() -> {
               while (true) {
                   synchronized (lock) {
                       ...
                       System.out.println("Verde: adelante por 4 segundos");
                       try {
                           Thread.sleep(4000);
                       } catch (InterruptedException e) {
                           e.printStackTrace();
           ...
           Thread hiloAmarillo = new Thread(() -> {
               while (true) {
                   synchronized (lock) {
                       ...
                       System.out.println("Amarillo - Precaución 2 segundos");
                       try {
                           Thread.sleep(2000);
                       } catch (InterruptedException e) {
                           e.printStackTrace();
